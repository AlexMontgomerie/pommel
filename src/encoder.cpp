...
template<>
encoder<general>::encoder(std::string config_path, std::string featuremap, platform_config_t platform) : platform(platform) {
    ...
    float compression_ratio = layer.select_node("parameter[@id='compression_ratio']").node().attribute("value").as_float();

    // get activity
    float activity = layer.select_node("parameter[@id='activity']").node().attribute("value").as_float();

    // initialise the coding scheme
    coder = new general(platform, compression_ratio, activity, batch_size);
